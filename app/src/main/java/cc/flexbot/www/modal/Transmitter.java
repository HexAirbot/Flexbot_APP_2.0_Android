...
import cc.flexbot.www.modal.OSDCommon.MSPCommnand;
...
public class Transmitter implements OSDDataDelegate {
	...
	private void transmmit() {
		 ...
		 * handler.post(new Runnable() {
		 * 
		 * @Override public void run() { Log.e("TEST", "handler.post");
		 * 
		 * updateDataPackage(); if (bleConnectionManager != null &&
		 * bleConnectionManager.isConnected() && dataPackage != null) {
	...
	private void updateDataPackage() {
		...
		/*
		 * if (check == 0) { channelList[0] = -0.5f; check = 1; } else{
		 * channelList[0] = 0.5f; check = 0; }
		 */
	...
	public void osdDataDidUpdateOneFrame() {
		Indicator deviceBatteryIndicator = HexMiniApplication.sharedApplicaion().getdeviceBatteryIndicator();
		float vbat = osdData.getVBat();
		Log.i("osdDataDidUpdate"," "+ vbat);
		double energy = 0;
		...
		if (energy <= 1.0 && energy >= 0.8) {
			deviceBatteryIndicator.setValue(4);
		} else if (energy < 0.8 && energy >= 0.6) {
			deviceBatteryIndicator.setValue(3);
		} else if (energy < 0.6 && energy >= 0.4) {
			deviceBatteryIndicator.setValue(2);
		} else if (energy < 0.4 && energy >= 0.2) {
			deviceBatteryIndicator.setValue(1);
		} else if (energy < 0.2) {
			deviceBatteryIndicator.setValue(0);
		}
