...
import cc.flexbot.www.ui.gl.GLSprite;
...
public abstract class JoystickBase extends Sprite {
	...
	public void draw(Canvas canvas) {
		...
		if (thumbNormal != null)
			thumbNormal.onDraw(canvas, thumbCenterX - (thumbNormal.width >> 1),
					inverseY(thumbCenterY - (thumbNormal.height >> 1)));
	}
	...
	public void moveThumbTo(float x, float y) {
		 ...
		 * 
		 * double distance = Math.sqrt(dx*dx + dy*dy); double angle =
		 * Math.atan2(dy, dx);
		 * 
		 * float joy_radius = bg.width / 2.0f - thumbNormal.width * 0.33f / 2;
		 * 
		 * if (distance > joy_radius) { dx = Math.cos(angle) * joy_radius; dy =
		 * Math.sin(angle) * joy_radius;
		 * 
		 * this.thumbCenterX = centerX + (float) dx; this.thumbCenterY = centerY
		 * + (float) dy; } else { this.thumbCenterX = x; this.thumbCenterY = y;
		 * }
		 */
	...
	protected void onActionDown(float x, float y) {
		...
		Log.d("onActionDown before", "centerX:" + centerX + ";centerY:" + centerY + ";thumbCenterX:" + thumbCenterX
		...
		if (analogueListener != null) {
			if (yStickIsBounced) {
				...
				if ((centerY - inverseY(y)) > (bg.height / 2.0f * yDeadBand)) {
					float percent = (centerY - inverseY(y) - (bg.height / 2.0f * yDeadBand))
							/ (yValidBand / 2.0f * bg.height);
					...
					float percent = (inverseY(y) - centerY - (bg.height / 2.0f * yDeadBand))
							/ (yValidBand / 2.0f * bg.height);
		...
		Log.d("onActionDown after", "centerX:" + centerX + ";centerY:" + centerY + ";thumbCenterX:" + thumbCenterX
				+ ";thumbCenterY:" + thumbCenterY);
	}
	...
	protected void onActionMove(float x, float y) {
		...
		// Log.d("onActionMove", "thumbCenterX:"+ thumbCenterX +
		// ";thumbCenterY:"+ thumbCenterY);
	...
	protected void onActionUp(float x, float y) {
		...
		Log.d("onActionUp", "centerX:" + centerX + ";centerY:" + centerY + ";thumbCenterX:" + thumbCenterX
				+ ";thumbCenterY:" + thumbCenterY);
	}
	...
	/*
	 * private float getXValue(float centerX, float x, float radius) { return -1
	 * * ((centerX - x) - (radius - (controlRatio * (radius * 2)))) /
	 * ((controlRatio * (radius * 2))); }
	 * 
	 * 
	 * private float getYValue(float centerY, float y, float radius) { return -1
	...
	public void setYValue(float yValue) {
		...
		// absolutMoveThumbTo(baseX, inverseY(baseY - (thumbCenterY -
		// centerY)));
